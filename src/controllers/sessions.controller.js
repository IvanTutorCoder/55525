...
export class SessionsController {
	...
	static failSignup = (req, res) => {
		res.send(
			"<p>Hubo un error al registrar al usuario <a href='/signup'>Intente de nuevo</a></p>"
		);
	};
	...
	static failLogin = (req, res) => {
		res.send(
			"<p>Hubo un error al iniciar sesion <a href='/login'>Intente de nuevo</a></p>"
		);
	};
	...
	static logoutUser = (req, res) => {
		req.session.destroy((err) => {
			if (err)
				return res.send(
					"<p>No fue posible cerrar sesion <a>intente de nuevo</a></p>"
				);
			res.redirect("/");
		});
	};
	...
	static resetPassword = async (req, res) => {
		try {
			...
			if (!userEmail) {
				return res.send(
					"El enlace caduco, <a href='/forgot-password'>genera un nuevo enlace</a>"
				);
			}
